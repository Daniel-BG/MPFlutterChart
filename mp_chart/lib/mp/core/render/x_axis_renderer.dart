...
class XAxisRenderer extends AxisRenderer {
  ...
  XAxisRenderer(
    ...
    if (_xAxis!.cacheAxisLabels && textPainterCache == null) {
  ...
  void _refreshTextPainterCacheStyle(
      double textSize, Color color, String? family, FontWeight? weight) {
    if (_cachedTextSize != textSize ||
        _cachedColor != color ||
  ...
  void computeSize() {
    ...
    if (textPainterCache != null) {
      _refreshTextPainterCacheStyle(xAxis!.textSize, _xAxis!.textColor,
          _xAxis!.typeface?.fontFamily, _xAxis!.typeface?.fontWeight);
      TextPainter tp = textPainterCache!.get(longest);
      labelSize = FSize.getInstance(0, 0);
      labelSize.width = tp.width;
      labelSize.height = tp.height;
      labelHeight = textPainterCache!.get("Q").height;
    } else {
      labelSize = Utils.calcTextSize1(axisLabelPaint!, longest);
      labelHeight = Utils.calcTextHeight(axisLabelPaint!, "Q").toDouble();
    }
    ...
    for (int i = 0; i < positions.length; i += 2) {
      ...
      if (viewPortHandler!.isInBoundsX(x)) {
        ...
        if (_xAxis!.avoidFirstLastClipping) {
          double width = 0;
          // get label width once layed out
          if (textPainterCache != null) {
            TextPainter tp = textPainterCache!.get(label);
            width = tp.width;
          } else {
            width = Utils.calcTextWidth(axisLabelPaint!, label).toDouble();
          }
  ...
  void drawLabel(Canvas c, String formattedLabel, double? x, double y,
    ...
    //TODO send cache to function so it can use it
